Feature: Vehicle Charging
  ...
  Background:
    ...
    And a charger "CHG-001" with type DC is "available"


  Scenario: Successful charging session
    When customer "Alissa" starts charging at "CHG-001" for 20 minutes
    Then the charging session for "Alissa" at "CHG-001" is completed
  ...
  Scenario: Charger becomes occupied during charging
    When customer "Alissa" starts charging at "CHG-001" for 10 minutes
    Then charger "CHG-001" status is "occupied"

  Scenario: Error - start charging when point is out of service
    Given a charger "CHG-001" with type DC is "out of service"
    When the customer tries to start a charging session at "CHG-001"
  ...
  Scenario: Error – customer has insufficient balance
    ...
    When customer "Eduard" starts charging at "CHG-001" for 60 minutes
